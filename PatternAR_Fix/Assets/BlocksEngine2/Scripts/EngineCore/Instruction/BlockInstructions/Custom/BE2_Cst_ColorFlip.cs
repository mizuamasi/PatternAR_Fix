...
public class InvertColorBlock : BE2_InstructionBase, I_BE2_Instruction
{
    ...
    {
        InvertTextureColorBasedOnCondition();
        ExecuteNextInstruction();
    }

    private void InvertTextureColorBasedOnCondition()
    {
        ...
        for (int x = 0; x < targetTexture.width; x++)
        {
            for (int y = 0; y < targetTexture.height; y++)
            {
                ...
                if (maskValue == 1.0f)  // 赤要素が1.0の場所で色を反転
                {
                    Color originalColor = tempTexture.GetPixel(x, y);
                    Color invertedColor = new Color(1.0f - originalColor.r, 1.0f - originalColor.g, 1.0f - originalColor.b, originalColor.a);
                    tempTexture.SetPixel(x, y, invertedColor);
                }
            }
        }
