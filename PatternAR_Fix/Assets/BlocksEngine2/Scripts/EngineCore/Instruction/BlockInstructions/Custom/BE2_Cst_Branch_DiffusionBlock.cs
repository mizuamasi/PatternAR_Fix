...
public class Branch_DiffusionBlock : BE2_InstructionBase, I_BE2_Instruction
{
    ...
    public new void Function()
    {
        ...
        // シェーダー処理を行い、結果をbufferTextureに保存
        Graphics.Blit(targetTexture, bufferTexture, processingMaterial);

        // bufferTextureに対して乗算を行い、最終結果をconditionTextureに保存
        multiplyMaterial.SetTexture("_SecondTex", bufferTexture);
        Graphics.Blit(targetTexture, conditionTexture, multiplyMaterial);
